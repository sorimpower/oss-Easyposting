...
(function () {
    ...
    /**
     * /(?:\{(i|c|b|data-([a-zA-Z0-9_-]+))\=\"([^\"]+)\"([\d\D\s\S\w\W\b\B]*)\})/gi
    ...
    InlineLexer.prototype.output = function (src) {
        ...
        while (src) {
            ...
            if (cap = this.rules.url.exec(src)) {
                ...
                var Google = mediaParseIdFromUrl('maps', href);
                ...
                if (YouTube !== undefined) {
                    out = '<iframe width="100%" height="280" src="//www.youtube.com/embed/' + YouTube + '" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
                    continue;
                } else if (Google !== undefined) {
                    out = '<iframe width="100%" height="280" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="' + Google + '"></iframe>';
                    continue;
                } else if (Vimeo !== undefined) {
                    out = '<iframe width="100%" height="280" src="//player.vimeo.com/video/' + Vimeo + '?api=1" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
                    continue;
                } else if (Viddler !== undefined) {
                    out = '<iframe width="100%" height="280" src="//www.viddler.com/player/' + Viddler + '" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
                    continue;
                } else if (DailyMotion !== undefined) {
                    out = '<iframe width="100%" height="280" src="//www.dailymotion.com/embed/video/' + DailyMotion + '" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>';
                    continue;
                }/* else if (Html5) {
                 //not working, make all urls to html5 video tag
                 out = '<video width="500" height="280" controls><source src="'+Html5['link']+'" type="video/'+Html5['extension']+'">Your browser does not support the video tag.</video>';
                 continue;
                 }*/ else {
                    out = '<a href="'
                            + href
    ...
    Renderer.prototype.image = function (href, title, text) {
        var out = '<img width="100%" src="' + href + '" alt="' + text + '"';
        if (title) {
            out += ' title="' + title + '"';
        }
