...
$(document).ready(function () {
    ...
    $('#custom0').click(function () {
        var keyword_selection = window.getSelection();
        var selection_text = keyword_selection.toString();
        if (selection_text != "") {
            $('#Keyword_Direct').val(selection_text);
...
$(function () {
    ...
    $('#Recommend').click(function () {
        ...
        $.ajax({
            ...
            if (data.success === true) {
                var sb = data.result_sb;
                var sp_sb = sb.split('|');
                var show = "";
                for (var i = 0; i < sp_sb.length; i++) {
                    if (i > 5)
                        break;
                    show += sp_sb[i] + ", ";
                }
