    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> ForgotPassword(ForgotPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    ModelState.AddModelError("", "사용자가 없거나 확인되지 않았습니다.");
                    return View();
                }
        ...
        public async Task<ActionResult> Manage(ManageUserViewModel model)
        {
            ...
            {
                // 사용자에게 암호가 없으므로 누락된 OldPassword 필드로 인해 생성된 모든 유효성 검사 오류를 제거하십시오.
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        // string code = await UserManager.GenerateEmailConfirmationTokenAsync(user.Id);
                        // var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
                        // SendEmail(user.Email, callbackUrl, "계정 확인", "이 링크를 클릭하여 계정을 확인하십시오.");
        ...
        {
            // 메일 보내기에 대한 자세한 내용은 http://go.microsoft.com/fwlink/?LinkID=320771을 참조하십시오.
        }
